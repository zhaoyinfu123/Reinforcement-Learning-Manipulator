...
def main():
    ...
    for eps in range(30):
        ...
        time1 = time.time()
        distance = dobot.dist_calculate(init_x, init_y, init_z, obj_x, obj_y, obj_z)

        worksheet_d.write(eps, eps_steps, float(distance))
        worksheet_x.write(eps, eps_steps, float(init_x - obj_x))
        worksheet_y.write(eps, eps_steps, float(init_y - obj_y))
        worksheet_z.write(eps, eps_steps, float(init_z - obj_z))
        ...
        while True:
            ...
            if moved == 1:
                ...
                eps_rewards += reward
                worksheet_d.write(eps, eps_steps, float(distance))
                worksheet_x.write(eps, eps_steps, float(end_x - obj_x))
                worksheet_y.write(eps, eps_steps, float(end_y - obj_y))
                worksheet_z.write(eps, eps_steps, float(end_z - obj_z))
        ...
        #     agent.save()
        # worksheet.write(eps, 0, eps+1)
        # worksheet.write(eps, 1, (str(obj_x)+','+str(obj_y)+','+str(obj_z)))
        # worksheet.write(eps, 2, round(eps_rewards, 3))
        # worksheet.write(eps, 3, eps_steps)
        # worksheet.write(eps, 4, RL_time)
        # 使用RL从init到obj

        workbook.save('/home/zyf/dobot_ws/models_and_rewards/plot_reward/RL_arm_test.xls')
