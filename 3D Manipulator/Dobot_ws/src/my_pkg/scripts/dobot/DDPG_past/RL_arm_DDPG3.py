...
class arm_control():
    ...
    def dist_calculate(self, obj_x, obj_y, obj_z):
        end_x, end_y, end_z = self.get_eof_location()
        dist = np.sqrt((end_x-obj_x)**2 + (end_y-obj_y)**2 + (end_z-obj_z)**2)
        return dist
    ...
    def get_state(self, obj_x, obj_y, obj_z):
        ...
        end_x, end_y, end_z = self.get_eof_location()
        dist = self.dist_calculate(obj_x, obj_y, obj_z)
        if dist < 0.1:
            goal = 1
        else:
            goal = 0
    ...
    for eps in range(max_episode):
        ...
        while True:
            ...
            if moved == 1:
                ...
                distance = dobot.dist_calculate(obj_x, obj_y, obj_z)
                reward = -distance*10
