...
def main():
    ...
    dqn = DQN()
    # dqn.load()
    workbook1 = xlwt.Workbook('utf-8')
    worksheet1 = workbook1.add_sheet('eps_rewards')
    workbook2 = xlwt.Workbook('utf-8')
    worksheet2 = workbook2.add_sheet('running_steps')
    for eps in range(EPISODES):
        ...
        while True:
            eps_steps += 1
            # 获得action
            action_out = dqn.choose_action(state)
            action = [action_out[0], action_out[1], action_out[2]]
            # 执行action
            ...
            if dqn.memory_counter >= MEMORY_CAPACITY and moved == 1:
                dqn.learn()
            if done:
                break
            state = next_state
            ...
            if eps_steps > 200:
                break
        dqn.save()
        worksheet1.write(eps, 0, round(ep_reward, 3))
        workbook1.save('/home/zyf/dobot_ws/src/plot_reward/DQN_rewards.xls')
        worksheet2.write(eps, 0, eps_steps)
        workbook2.save('/home/zyf/dobot_ws/src/plot_reward/DQN_steps.xls')
