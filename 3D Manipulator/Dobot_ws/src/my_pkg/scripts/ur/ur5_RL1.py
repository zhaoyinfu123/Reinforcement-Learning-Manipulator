...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.l1 = nn.Linear(state_dim + action_dim, 1000)
        self.l2 = nn.Linear(1000, 800)
...
class DDPG(object):
    ...
    def save(self):
        torch.save(self.actor.state_dict(), '/home/zyf/dobot_ws/models_and_rewards/models/ur5_DDPG_actor.pth')
        torch.save(self.critic.state_dict(), '/home/zyf/dobot_ws/models_and_rewards/models/ur5_DDPG_critic.pth')

    def load(self):
        self.actor.load_state_dict(torch.load('/home/zyf/dobot_ws/models_and_rewards/models/ur5_DDPG_actor.pth'))
        self.critic.load_state_dict(torch.load('/home/zyf/dobot_ws/models_and_rewards/models/ur5_DDPG_critic.pth'))
        print("====================================")
...
def main():
    ...
    for eps in range(max_episode):
        ...
        traj, _, _, _ = ur5.verify_executable()
        print('Go to a random location')
        ur5.execut(traj)

        obj_x, obj_y, obj_z = 0.5, 0.5, 0.5
        state = ur5.get_state(obj_x, obj_y, obj_z)

        while True:
            action = agent.select_action(state)
            joint_value = ur5.get_joint_value()
            joint_value = [joint_value[0]+action[0]*dt, joint_value[1]+action[1]*dt,
                           joint_value[2]+action[2]*dt, joint_value[3]+action[3]*dt,
                           joint_value[4]+action[4]*dt, 0]
            moved = ur5.set_joint_value(joint_value)
            if moved == 1:
                next_state = ur5.get_state(obj_x, obj_y, obj_z)
                end_x, end_y, end_z = ur5.get_eof_locaton()
                distance = ur5.dist_calculate(end_x, end_y, end_z, obj_x, obj_y, obj_z)
                reward = -distance*10
                if distance < 0.02:
                    done = True
                else:
        ...
        workbook.save('/home/zyf/dobot_ws/models_and_rewards/plot_reward/ur5_DDPG_rewards.xls')
